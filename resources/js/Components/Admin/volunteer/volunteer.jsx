...
import { fetchVolunteerAssignments, addVolunteerAssignment, editVolunteerAssignment, cancelAssignment, getMe, updateAssignmentStatus, capitalize } from "@/Utils/utils";
...
const VolunteerAssignmentsPage = () => {
    ...
    const [viewMode, setViewMode] = useState(false); // State to handle View mode
    ...
    const tableHeaders = ['ID', 'Task', 'Volunteer', 'Assigned Date', 'Status', 'Actions'];
    ...
    const tableData = assignments.map((assignment) => [
        ...
        assignment.assigned_at || 'No Date',
        <Dropdown
        ...
        <Dropdown
            options={options}
            onSelect={(action) => handleAction(action, assignment)} // Handle action selection
        />,
    ]);
